...
def tim_kiem_web(so_ky_hieu, dia_phuong="tphcm"):
    base = VBPL_DOMAIN.get(dia_phuong, "{}/{}".format(VBPL_BASE, dia_phuong))
    url = "{}/Pages/vbpq-tim-kiem.aspx".format(base)
    ...
    try:
        ...
        for item in items[:5]:
            ...
            tt_el = item.select_one(".trang-thai, .hieu-luc")
            results.append({
                "id": id_match.group(1) if id_match else None,
                "ten": link.get_text(strip=True),
                "so_ky_hieu": so_ky_hieu,
                "trang_thai": tt_el.get_text(strip=True) if tt_el else "",
                "url": href if href.startswith("http") else "{}{}".format(base, href),
            })
        return results
    except Exception as e:
        logger.error("[WEB] Scraping loi: {}".format(e))
        return []
...
def lay_chi_tiet_web(url):
    try:
        ...
        for row in soup.select(".table-info tr, .vbpq-info tr, table tr"):
            ...
            if "so ky hieu" in key or "số ký hiệu" in key:
                detail["so_ky_hieu"] = val
            elif "ngay ban hanh" in key or "ngày ban hành" in key:
                detail["ngay_ban_hanh"] = val
            elif "co quan" in key or "cơ quan" in key:
                detail["co_quan_ban_hanh"] = val
            elif "hieu luc" in key or "hiệu lực" in key:
                detail["ngay_hieu_luc"] = val
            elif "trang thai" in key or "trạng thái" in key:
                detail["trang_thai"] = val
            elif "loai van ban" in key or "loại văn bản" in key:
                detail["loai_van_ban"] = val
        files = []
        for a in soup.select('a[href$=".pdf"], a[href$=".doc"], a[href$=".docx"]'):
            href = a.get("href", "")
            files.append({
                "ten": a.get_text(strip=True) or "Tai file",
                "url": href if href.startswith("http") else "{}{}".format(VBPL_BASE, href),
                "loai": href.rsplit(".", 1)[-1].upper(),
            })
        detail["files"] = files
        detail["van_ban_tac_dong"] = []
        return detail
    except Exception as e:
        logger.error("[WEB] Chi tiet loi: {}".format(e))
        return {}
...
def tra_cuu_van_ban(so_ky_hieu, dia_phuong="tphcm"):
    ...
    for r in results[:3]:
        ...
        if vb_id:
            ...
            r.update(dict((k, v) for k, v in detail.items() if v))
            r["files"] = lay_file_dinh_kem(vb_id)
            r["van_ban_tac_dong"] = lay_van_ban_tac_dong(vb_id)
        elif r.get("url"):
            web_detail = lay_chi_tiet_web(r["url"])
            r.update(dict((k, v) for k, v in web_detail.items() if v))
        enriched.append(r)
    return enriched

def xac_dinh_trang_thai(trang_thai):
    ts = trang_thai.lower()
    if any(x in ts for x in ["con hieu luc", "còn hiệu lực"]):
        return "✅", "CON HIEU LUC"
    if any(x in ts for x in ["mot phan", "một phần"]):
        return "⚠️", "HET HIEU LUC MOT PHAN"
    if any(x in ts for x in ["het hieu luc", "hết hiệu lực"]):
        return "❌", "HET HIEU LUC"
    if any(x in ts for x in ["chua", "chưa"]):
        return "🕐", "CHUA CO HIEU LUC"
    return "❓", trang_thai or "Khong xac dinh"
